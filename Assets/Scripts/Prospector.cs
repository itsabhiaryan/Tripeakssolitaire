...
{
    static public Prospector Instance;
    ...
    public Vector2 fsPosMid = new Vector2(0.5f, 0f);
    public Vector2 fsPosRun = new Vector2(0.5f, 0f);
    ...
    void Awake()
    {
        Instance = this;//set up a singleton for prospector    
    }
    void Start()
    {
        Scoreboard.Instance.score = ScoreManager.SCORE;
    ...
    private CardProspector animateObject;
    private float threshold = 0.5f;
    private float speed = 20.0f;

    void Update()
    {
        if (animateObject != null && targetPosition != Vector3.zero)
        {
            Vector3 direction = targetPosition - animateObject.transform.localPosition;
            if (direction.magnitude > threshold) {
                direction.Normalize();
                animateObject.transform.localPosition = animateObject.transform.position + direction * speed * Time.deltaTime;
            }
            else
            {
                animateObject.transform.position = targetPosition;
                targetPosition = Vector3.zero;
                animateObject = null;
            }
        }
    ...
    void MoveToTarget(CardProspector cd, bool animate)
    {
        ...
        if (animate) 
        {
            ...
            targetPosition = new Vector3(layout.multiplier.x * layout.discardPile.x, layout.multiplier.y * layout.discardPile.y, -layout.discardPile.LayerID);
        }
        else
    ...
    public void CardClicked(CardProspector cd)
    {
        ...
        switch (cd.state)
        {
            ...
            case eCardState.tableau:
                ...
                MoveToTarget(cd, true);//Make it target card
                SetTableauFaces();//Update tableau card face-ups
                ScoreManager.EVENT(eScoreEvent.mine);
                Scoreboard.Instance.UpdateScore(ScoreManager.CHAIN);
                break;
        }
    ...
    public void GameOver(bool won)
    {
        ...
        SceneManager.LoadScene("TripeaksSolitaire");
        // Submit score 
    }
    ...
    void FloatingScoreHandler(eScoreEvent evt)
    {
        ...
        switch (evt)
        {
            ...
            case eScoreEvent.mine:
                ...
                fsPts.Add(fsPosMid);
                fsPts.Add(fsPosRun);
                fs = Scoreboard.Instance.CreateFloatingScore(ScoreManager.CHAIN, fsPts);
